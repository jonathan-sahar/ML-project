...
def divideToWindows(dataMatrix, windowLength):
    '''
    :dataMatrix: array representing a chunk of data
    :returns: a list of (long) window objects.
    '''
    windows =[]
    window = Window()
    lineCounter = 0
...
class Window:
    def __init__(self):
        self.data = []
        self.shortWindowIndices = []

    def appendRow(self, row):
        self.data.append(row)

    def divideToShortWindows(self, row):
        self.shortWindowIndices = [(x, x + SHORT_TIME_WINDOW) for x in range(len(self.data) - SHORT_TIME_WINDOW)]
...
if __name__ == "__main__":
    ...
    # Do stuff with entire data
    dataMatrix = deque(dataMatrix)
    longWindows = divideToWindows(dataMatrix, LONG_TIME_WINDOW)
    aggregate(aggregatorsListLong, TimeWindow='long', longWindows, shortTimeMatrix, None)
